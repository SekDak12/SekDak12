...
CML2Ph_FullWater::CML2Ph_FullWater()
{
  ...
  halfSize.set(150.*mm,3000.*mm,3000.*mm);
  //halfSize.set(150.*mm,3000.*mm,3000.*mm);

  // phantom position
  centre.set(0.,0.,0.);
  //centre1.set(1000.,0.,0.);
  
  fPhantomSize.setX(300.*mm);
  fPhantomSize.setY(6000.*mm);
  fPhantomSize.setZ(6000.*mm);
...
bool CML2Ph_FullWater::Construct(G4VPhysicalVolume *PWorld, G4int nx, G4int ny, G4int nz)
{
 ...
 bool bCreated=false;
 G4Material *WATER=G4NistManager::Instance()->FindOrBuildMaterial("G4_Pb");
 G4Box *fullWaterPhantomBox = new G4Box("fullWaterPhantomBox", halfSize.getX(), halfSize.getY(), halfSize.getZ());
 G4LogicalVolume *fullWaterPhantomLV = new G4LogicalVolume(fullWaterPhantomBox, WATER, "fullWaterPhantomLV", 0, 0, 0);//////////PONERLE UN SD
 fullWaterPhantomPV = new G4PVPlacement(0, centre, "fullWaterPhantomPV", fullWaterPhantomLV, PVWorld, false, 0);
  ...
  G4VSolid* solVoxel = new G4Box(zVoxName,sensSize.x()/2.,sensSize.y()/2.,sensSize.z()/2.);///////////////VOLUMEN NO POSICION
  
  G4LogicalVolume* LVPhantomSens = new G4LogicalVolume(solVoxel,WATER,zVoxName); // This is the Sensitive Volume
  PhantomSensPV = new G4PVPlacement(0, centre, "PhantomSensPV", LVPhantomSens, PVWorld, false, 0);

  new G4PVReplica(zVoxName,LVPhantomSens,logXRep,kZAxis,nz,sensSize.z(),-3200.);/////CAMBIAR EL EJE DE REPLICA!!!!!!!!!!!!!!!!!!!!!!!!!111
//..............................................
  ...
  regVol->SetProductionCuts(cuts);
	...
	simpleAlSVisAtt->SetVisibility(true);
 	simpleAlSVisAtt->SetForceSolid(false);
	...
	LVPhantomSens->SetVisAttributes(simpleAlSVisAtt);
